...
class Tests(unittest.TestCase):
    ...
    def test_parse_line(self):
        result = parse_line(self.example_lines[0])
        expect = (
            1,
            [41,48,83,86,17],
    ...
    def test_solve1(self):
        scratchcards = list(map(parse_line, self.example_lines))
        result = solve1(scratchcards)
        expect = 13
        self.assertEqual(expect, result)

    def test_solve2(self):
        scratchcards = list(map(parse_line, self.example_lines))
        result = solve2(scratchcards)
        expect = 30 # total scratchcards
        self.assertEqual(expect, result)


cardline_re = re.compile(r'Card +(\d+): ([\d ]+) \| ([\d ]+)')

def parse_line(line):
    match = cardline_re.match(line)
    card_number, *number_groups = match.groups()
    card_number = int(card_number)
    winners, numbers = [
        list(map(int, number_group.split()))
        for number_group in number_groups
    ]
    return (card_number, winners, numbers)

def winning_numbers(scratchcard):
    card, winners, numbers = scratchcard
    return set(winners).intersection(numbers)

def points(scratchcard):
    points = int(2 ** (len(winning_numbers(scratchcard)) - 1))
    return points

def solve1(scratchcards):
    return sum(map(points, scratchcards))

def solve2(scratchcards):
    instances = {card: 1 for card, _, _ in scratchcards}
    nwinners = {
        scratchcard[0]: len(winning_numbers(scratchcard))
        for scratchcard in scratchcards
    }
    for card, _, _ in sorted(scratchcards, key=itemgetter(0)):
        nwins = nwinners[card]
        if not nwins:
            continue
...
def main(argv=None):
    ...
    parser.add_argument(
        '-i', '--input',
        default = 'instance/inputs/day04.txt',
        type = argparse.FileType(),
    )
    args = parser.parse_args(argv)

    with args.input as input_file:
        scratchcards = list(map(parse_line, map(str.strip, input_file)))

    part1_answer = solve1(scratchcards)
    assert part1_answer == 17782
    print(f'{part1_answer=}')

    part2_answer = solve2(scratchcards)
