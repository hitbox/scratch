    ...
    def __init__(self):
        ...
        for y, row in enumerate(self.buttons):
            for x, _ in enumerate(row):
                if self.buttons[y][x] == 5:
                    self.position = (x, y)
                    break
        assert self.position is not None

        self.nrows = len(self.buttons)
        self.ncols = len(self.buttons[0])

        self.pressed = []

    def isvalid(self, move):
        nx, ny = self.nextposition(move)
        if self.ncols > nx > -1 and self.nrows > ny > -1:
            return (nx, ny)

    def nextposition(self, move):
        mx, my = self.MOVE[move]
        x, y = self.position
        return (x + mx, y + my)

    def process(self, instructions):
        log = logging.getLogger('process')
        log.debug('started at %s' % (self.position, ))
        for line in instructions.splitlines():
            for move in line:
                log.debug('move: %s' % move)
                rv = self.isvalid(move)
                if rv is not None:
                    self.position = rv
                    log.debug('self.position updated %s' % (self.position, ))
            x, y = self.position
            self.pressed.append(self.buttons[y][x])
            log.debug('saved button press: %s' % self.buttons[y][x])
        return ''.join(map(str, self.pressed))
...
class Keypad2(Keypad):
    ...
    def isvalid(self, move):
        rv = super(Keypad2, self).isvalid(move)
        if rv:
            x, y = rv
            if self.buttons[y][x] is not None:
                return rv
...
def tests():
    example = "ULL\nRRDDD\nLURDL\nUUUUD"
    assert Keypad().process(example) == '1985'
    assert Keypad2().process(example) == '5DB3'

def part1():
    instructions = load()
    print('Day 02, part 1: bathroom code is "%s"' % Keypad().process(instructions))

def part2():
    instructions = load()
    print('Day 02, part 2: bathroom code, with actual keypad, is "%s"' % Keypad2().process(instructions))
